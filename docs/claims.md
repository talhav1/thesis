...
| C2b | `entropy_sigma` does not reproduce in any setting | **Unresolved** | 250 reps/cell | `docs/discrepancies.md` D13, D26 | Criterion verified exact against a closed form (efficiency 1.000, corr 0.996), and the published table is provably inconsistent with the published criterion: her RMS error for $\mu$ is 1.34 from a prior 8 units off, which a design maximising $I(\sigma;Y\mid x)$ cannot achieve. Unresolved is now about *attribution* — text vs code — not about whether this implementation is right. Needs Rotem's code. **D26:** that exactness audit was step-0 only (its closed form needs $\mu\perp\sigma$); extended past the prior against the reference grid posterior, efficiency stays $\ge 0.978$ (worst single state 0.905) through step 30, so the estimator is exonerated along the whole trajectory and the attribution reading is unchanged. |
| C2c | `entropy_sigma`'s edge-pinning is a self-reinforcing trap, not merely a property of the criterion at the prior | **Established** | screening, 30 reps | `results/summaries/phase1_sigma_criterion_drift.csv`, `docs/discrepancies.md` D26 | The exact $\sigma$-optimum sits at a grid edge at every step under `entropy_sigma`, but interior ($\approx \mu + 1.2\sigma$) under `entropy_vector`. At step 30 there is 6.5× more $\sigma$-information available under the design that never targets $\sigma$ (0.0239 vs 0.0037 nats), because sd$(\mu)$ is 0.94 vs 4.75. Greedy one-step $\sigma$-information keeps $\mu$ vague, and vague $\mu$ keeps the optimum at the edge. Explains why this implementation's `entropy_sigma` is the worst design in the study at estimating $\sigma$ — 0/6 cells reproduce on the $\sigma$ target while every other policy reproduces 6/6. One cell only; not swept over $\sigma_0$ or the misspecified curves. |
